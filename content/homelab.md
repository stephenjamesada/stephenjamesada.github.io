+++
title = 'Homelab'
date = 2026-06-10T08:47:12-05:00
draft = false
+++

<img src="/images/thinkpad-3.jpg" alt="ThinkPad Open" height="725" style="width: 725px;">

Primary homelab machine; used for self-hosting, administration practice, and infrastructure experimentation.

## Hardware

- **Machine**: Lenovo ThinkPad T560
- **OS**: Ubuntu Server (Noble Numbat; 24.04.4 LTS)
- **CPU**: Intel Core i5-6200U
- **RAM**: Kingston 16GB DDR3
- **Storage**: Intel DC S3700-Series 1TB Enterprise SATA SSD

## Services Hosted

- Gitea

## Related Repositories

- [Homelab Infrastructure](https://github.com/stephenjamesada/homelab-infrastructure)
- [Homelab Notes](https://github.com/stephenjamesada/homelab-notes)
